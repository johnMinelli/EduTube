                    ...
                    bookmarks.keys.forEach {
                        query2.document(it).get().addOnSuccessListener { documentSnapshot ->
                            try {
                                ...
                                Log.d("Tag.BookmarksViewModel", "Cast gone bad")
                            }
                        }.addOnFailureListener { exception ->
                            Log.d("Tag.BookmarksViewModel", "First level exception ${exception}")
                            //probabily a deleted content
                            bookmarks.remove(it)
                            query.update("bookmarks",bookmarks)
                        }
                    }
                }
            }
        }catch(e: Exception){
            Log.d("Tag.BookmarksViewModel","Cast mutable bookmarks gone bad")
        }
    }
